# ⚜️ Latoka Naming
- [Names](#names)
  - [Male](#male)
  - [Female](#female)
- [Surnames](#surnames)

## Names
Latoka names have some common naming schemes, e.g. `In-`, `-t`.  
It's also worth noting that there's common differentiation between genders by assigning
`e` to masculine form and `a` to feminine. Coincidentally this also follows the same pattern
as ghloddish naming does, albeit the position of those wovels is inside name, not at the beginning.

### Male

| Name        | Rarity         | Description                     |
|-------------|----------------|---------------------------------|
| Anter       | 🔆 Semi-common |                                 |
| Anterkiritt | ⚜️ Rare        | Combination of Anter and Kiritt |  
| Iner        | 🔥 Common      | Has feminine form (Inar)        |
| Inarkin     | 🏵️ Semi-rare  | Baedoorised form of Iner        |
| Inluet      | 🔆 Semi-common | Has feminine form (Inluat)      |
| Inluern     | 🏵️ Semi-rare  | Baedoorised form of Inluet      |
| Kiritt      | 🔆 Semi-common |                                 |
| Nasvioh     | 🏵️ Semi-rare  |                                 |
| Satarap     | 🔆 Semi-common |                                 |
| Tokkot      | 🏵️ Semi-rare  |                                 |

### Female

| Name    | Rarity         | Description                 |
|---------|----------------|-----------------------------|
| Inar    | 🔥 Common      | Has masculine form (Iner)   |
| Inluat  | 🔆 Semi-common | Has masculine form (Inluet) |
| Lat     | 🔆 Semi-common |                             |
| Luat    | 🏵️ Semi-rare  | Mix of Lut and Lat names    |
| Lut     | 🔆 Semi-common |                             |
| Nel     | 🔆 Semi-common |                             |
| Nelvose | 🔆 Semi-common |                             |
| Onyri   | 🏵️ Semi-rare  |                             |

### Surname
| Name | Localisation                       | Description                            |
|------|------------------------------------|----------------------------------------|
| Ett  | Ilumat Highlands, Dewehr Mountains | Aristocratic/royal traditional surname |