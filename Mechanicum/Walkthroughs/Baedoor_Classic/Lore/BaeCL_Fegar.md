# 📿 Fegar County

---
**🪔 Baedoor** (2008)

---
### Fegar County Showcase

<table>
  <!--PHOTO 1-->
  <tr>
    <td width="420" rowspan="2"><img alt="Baedoor Archives, Photo 30" height="307" src="/Assets/Sources/001_Lnd__Baedoor/arv_baedoor_30.jpg" width="416"/></td>
    <th align="centre">Fegar Housing</th>
  </tr>
  <tr>
    <td>
      This page shows potter house, alongside jitadian merchant one. One can see Battle Mages influences once again in
      shape of merchant's vehicle.
    </td>
  </tr>
  <!--PHOTO 2-->
  <tr>
    <td width="420" rowspan="2"><img alt="Baedoor Archives, Photo 31" height="307" src="/Assets/Sources/001_Lnd__Baedoor/arv_baedoor_31.jpg" width="416"/></td>
    <th align="centre">Watchtower</th>
  </tr>
  <tr>
    <td>
      Contains mostly watchtower with big bell and house made out of skin, of possibly mesinpean
      inhabitant (or at least during archiving this seems as more possible option considering his
      culture of work & unusual name which I applied to mesinpean origin).
    </td>
  </tr>
  <!--PHOTO 3-->
  <tr>
    <td width="420" rowspan="2"><img alt="Baedoor Archives, Photo 32" height="307" src="/Assets/Sources/001_Lnd__Baedoor/arv_baedoor_32.jpg" width="416"/></td>
    <th align="centre">Hill Grove</th>
  </tr>
  <tr>
    <td>
      Small grove leading to Isuga's island nearby - contains quite typical architecture of woodhouse
      built in middle-to-late era in woodlands, mostly in Kaer, with also height-adjusting structure
      with entries on varied height levels.<br>
      Inhabited by carpenter of unusual (and possibly foreign) name.
    </td>
  </tr>
  <!--PHOTO 4-->
  <tr>
    <td width="420" rowspan="2"><img alt="Baedoor Archives, Photo 33" height="307" src="/Assets/Sources/001_Lnd__Baedoor/arv_baedoor_33.jpg" width="416"/></td>
    <th align="centre">Fegar Coast</th>
  </tr>
  <tr>
    <td>
      Coast separating Fegar County and island holding biggest monument near Isuga. Panel shows
      house of fisherman alongside his self-made wood boat.
    </td>
  </tr>
  <!--PHOTO 5-->
  <tr>
    <td width="420" rowspan="2"><img alt="Baedoor Archives, Photo 34" height="307" src="/Assets/Sources/001_Lnd__Baedoor/arv_baedoor_34.jpg" width="416"/></td>
    <th align="centre">Golden Lighthouse</th>
  </tr>
  <tr>
    <td>
      Panel holding biggest monument in Baedoor, <b><a href="/Loreum/Topics/Geography/Baedoor/Places/Golden_Lighthouse_of_Baedoor.md">
      Golden Lighthouse of Baedoor</a></b>.
    </td>
  </tr>
  <!--PHOTO 6-->
  <tr>
    <td width="420" rowspan="2"><img alt="Baedoor Archives, Photo 35" height="307" src="/Assets/Sources/001_Lnd__Baedoor/arv_baedoor_35.jpg" width="416"/></td>
    <th align="centre">Isuga Coast</th>
  </tr>
  <tr>
    <td>
      Other side of Fegar island coast. Contains merchant house and a church, which is quite odd for
      that unoccupied area, but fits well my typical chaotic drawing at the time (and also Isuga being
      way smaller than it claims to be).
    </td>
  </tr>
  <!--PHOTO 7-->
  <tr>
    <td width="420" rowspan="2"><img alt="Baedoor Archives, Photo 36" height="307" src="/Assets/Sources/001_Lnd__Baedoor/arv_baedoor_36.jpg" width="416"/></td>
    <th align="centre">Spirit Grove</th>
  </tr>
  <tr>
    <td>
      Small grove with Nature Spirit floating between trees. Pretty empty panel, however it has some
      warriors (possibly bandits) wandering there too.
    </td>
  </tr>
  <!--PHOTO 8-->
  <tr>
    <td width="420" rowspan="2"><img alt="Baedoor Archives, Photo 37" height="307" src="/Assets/Sources/001_Lnd__Baedoor/arv_baedoor_37.jpg" width="416"/></td>
    <th align="centre">Isuga Border</th>
  </tr>
  <tr>
    <td>
      Right before entering Isuga, there's hill with probably bandit warriors and some monsters. In
      normal circumstances, this would end up with fight between two, but here they seem to live pretty
      peacefully, awaiting the player and protecting chest. Monsters are called as *rebels* which also
      gives hint that they may have been bandit-aligned.<br>
      There's also some platformer inspiration in that flying entity that drops spike cubes, and the
      entity itself is called *echidna*, but does look more Mario-inspired.
    </td>
  </tr>
</table>

---
### Lore accuracy
Baedoor captured in **Baedoor Classic** is most incoherent of sourced materials, as it does have
several conflicting aspects with proclaimed time period (1008-1011 OE), namely:
- Tom Varteford being an adventurer, not yet king (which would apply to 966-998 OE)
- Baedoor looking as it would look in 200-300 OE, in most favourable view, 500 OE, but nowhere near
  said timeframe
- Isuga being desert city, whereas it always stayed on grasslands

But, in the same time, it is considered as core resource for lore exploration.  
Hence, sources on Baedoor from **Baedoor Classic** are treated in specific scope:
- Coherent aspects are used in lore
- Incoherences related to historical events and climate are ignored (knowing my mindset from times of
  drawing those lands)
- Incoherences related to architecture are used in lore accordingly to timeframe (so if architecture
  follow 200 OE timeframe, it will be used as source for this period)

Notebook source: [**N.1.001_lnd**](/Mechanicum/Walkthroughs/Baedoor_Classic/Sources/001_Lnd__Baedoor.md)

---
[💠](/Entrance.md) 〰️ [➰](/Mechanicum/Mechanicum.md) 〰️ [📿](/Mechanicum/Walkthroughs/Baedoor_Classic/BaeCL_Lore.md)